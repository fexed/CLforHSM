from ascertain import ASCERTAINTrSet, ASCERTAINTsSet
...
def train_ascertain(strat, i=""):
	...
	text_logger = TextLogger(open('ascertainlog.txt', 'a'))
	...
	if (strat == "naive"):
		...
		strategy = Replay(model, Adam(model.parameters(), lr=0.005, betas=(0.99, 0.99)), CrossEntropyLoss(), train_epochs=100, eval_every=1, plugins=[es], evaluator=eval_plugin, device=device, mem_size=25, train_mb_size=25)  #circa 25% of ASCERTAIN
	elif (strat == "lwf"):
	...
	with open("results/ascertain_" + strat + "_results" + i + ".pkl", "wb") as outfile:
